...
// トップへ戻るボタン
// 画面を少し(今回は100px)スクロールした時に表示 (通常は非表示)
// かつ、少しだけfooterの上部からボタンの上部がはみ出す位置でSTOPするように
...
window.addEventListener("scroll", function () {
  ...
  if (scrollY > 100) {  // 100px
    ...
    // 少しだけfooterの上部からボタンの上部がはみ出す位置でSTOPするように
    if (scrollY + windowHeight > bodyHeight - footerHeight) {
      pageTop.style.bottom = `${scrollY + windowHeight - (bodyHeight - footerHeight) - 35}px`;  // ボタンの最下部の位置が、footerの上部より35px下になる位置で止める
    } else {
      pageTop.style.bottom = '40px';  // footerに近くない場合は、画面の下から40pxの位置に固定。
    }
  } else {
